...
def search_barcode():
    default_price_list = frappe.db.get_single_value("Barcode Updation Settings", "default_price_list")
    default_warehouse = frappe.db.get_single_value("Barcode Updation Settings", "default_warehouse")
    ...
    fields = frappe.db.sql("""Select a.website_item,a.item_code,a.web_item_name,a.website_image,a.uom,a.price_list_rate,b.country
        ...
        where `tabWebsite Item`.item_code=`tabItem Price`.item_code and `tabItem Price`.price_list='%(default_price_list)s')a
        RIGHT JOIN 
        (select `tabItem`.name,`tabItem`.country_of_origin as country from `tabItem`,`tabPOS Offer`  where `tabItem`.name=`tabPOS Offer`.item and `tabPOS Offer`.warehouse='%(default_warehouse)s')b ON (b.name=a.item_code) """,{"default_price_list":default_price_list,"default_warehouse":default_warehouse}, as_dict=1)
    return fields
...
LEFT JOIN `tabItem Price` b ON (a.parent=b.item_code and b.selling=1 and a.uom=b.uom and b.price_list='%(default_price_list)s') where a.idx=1)u1
LEFT JOIN
(Select a.parent as parent2,a.uom as uom2,b.price_list_rate as uom_price2 FROM `tabUOM Conversion Detail` a
LEFT JOIN `tabItem Price` b ON (a.parent=b.item_code and b.selling=1 and a.uom=b.uom and b.price_list='%(default_price_list)s') where a.idx=2)u2
...
LEFT JOIN `tabItem Price` b ON (a.parent=b.item_code and b.selling=1 and a.uom=b.uom and b.price_list='%(default_price_list)s') where a.idx=3)u3
...
LEFT JOIN `tabItem Price` b ON (a.parent=b.item_code and b.selling=1 and a.uom=b.uom and b.price_list='%(default_price_list)s') where a.idx=4)u4
ON(u1.parent=u4.parent4))uom_price
ON(item.item_code=uom_price.parent) where item.item_group='%(item_group)s' """%{"default_price_list":default_price_list,"item_group":item_group}, as_dict=1)
    return fields
...
LEFT JOIN `tabItem Price` b ON (a.parent=b.item_code and b.selling=1 and a.uom=b.uom and b.price_list='%(default_price_list)s') where a.idx=4)u4
ON(u1.parent=u4.parent4))uom_price
ON(item.item_code=uom_price.parent) where item.item_code='%(item)s' """%{"default_price_list":default_price_list,"item":item}, as_dict=1)
    return fields